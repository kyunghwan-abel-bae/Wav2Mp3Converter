...
MyWave::MyWave (const char* file_name) {
    fmt_.wFormatTag = 0;
    ...
    my_wave_.resize( data_.dataSIZE );

    file.read( &my_wave_[0], data_.dataSIZE );
}

int32_t MyWave::get_samples_per_sec() const {
    return fmt_.nSamplesPerSec;
}

int32_t MyWave::get_avg_bytes_per_sec() const {
    return fmt_.nAvgBytesPerSec;
}

int16_t MyWave::get_channels() const {
    return fmt_.nChannels;
}

void MyWave::get_samples( unsigned int offset, unsigned int size, std::vector<char>& samples) const {
    ...
    samples.insert( samples.end(), &my_wave_[offset], &my_wave_[offset+real_size] );
}
